...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if (!jaildekiler.some(j => j.id == uye.id)) {
    jdb.push(`jail`, {id: uye.id, kalkmaZamani: Date.now()+ms(sure)});
  ...
  if(ayar.jailLogKanali && client.channels.cache.has(ayar.jailLogKanali)) client.channels.cache.get(ayar.jailLogKanali).send(embed.setDescription .setImage("https://cdn.discordapp.com/attachments/736212915266388079/783761380641734737/C_.gif")(`${uye} üyesi, ${message.author} tarafından **${sure}** boyunca **${reason}** nedeniyle jaile atıldı!`)).catch();
};
